...
module.exports = async (req, res) => {
    try {
        ...
        let rawPlay = req.query && req.query.play ? req.query.play : null;
        if (!rawPlay) {
            let urlPath = req.url || '';
            if (urlPath.includes('.m3u8') || urlPath.includes('embed-') || urlPath.length > 1) {
                rawPlay = urlPath.replace(/^\/+/, '');
            }
        }
        ...
        if (rawPlay) {
            ...
            // Extract custom referer from the piped URL if present
            let customReferer = "";
            if (rawPlay.includes('Referer=')) {
                const refMatch = rawPlay.match(/Referer=([^&]+)/i);
                if (refMatch && refMatch[1]) {
                    customReferer = decodeURIComponent(refMatch[1]);
                }
            }
            ...
            // Agar ye streamoupload.xyz ka hai, toh seedha view source (HTML) dikha do
