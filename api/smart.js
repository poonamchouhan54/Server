...
const databaseURL = "https://ipl2020-46d2f.firebaseio.com";

const url = new URL(request.url);
const userToken = url.searchParams.get("token");
if (!userToken) {
  return new Response("Error: Token missing", { status: 403 });
}

try {
  // 1. Firebase ki device_tokens list me check karna
  const tokenCheckRes = await fetch(`${databaseURL}/device_tokens/${userToken}.json`);
  const tokenData = await tokenCheckRes.json();
  if (tokenData === null) {
    return new Response("Error: Token expired or invalid", { status: 403 });
  }

  // 2. Token mil gaya, turant list me se delete kar dena (Single-use)
  await fetch(`${databaseURL}/device_tokens/${userToken}.json`, {
    method: "DELETE"
  });

  // 3. Playlist fetch aur process karna
  ...
  for (let i = 0; i < otherChannels.length; i++) {  
    ...
    output.push(ch.extinf);  
    if (ch.extraMetadata.lengthยอด > 0) output.push(ch.extraMetadata.join("\n"));  
    output.push(ch.url);  
  }  
